...
const filterById = (geojson, id) =>
  geojson
    ? geojson.features.find((feat) => feat.properties.autoid === id)
    : false;
    ...
    <SidebarWrapper isvisible={isVisible.toString()}>
      ...
      <SidebarContent>
        <Switch>
          ...
          />
          <Route path="/liste" render={() => <SidebarList data={data} />} />
          <Route path="/info" render={() => <SidebarInfo />} />
          <Route path="/favoriten" render={() => <SidebarFav data={data} />} />
          <Route path="/suche" render={() => <SidebarFilter />} />
        </Switch>
      </SidebarContent>
    </SidebarWrapper>
  );
